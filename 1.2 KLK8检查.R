...
for (obj_name in seurat_objects) {  
  klk8_expression <- FetchData(object = get(obj_name), vars = "KLK8")  
  cat(sprintf("Object %s - KLK8 Expression:\n", obj_name))  
  print(klk8_expression)  
  cat("\n")  
}
...
####合并对象的KLK8#####################
#合并对象的KLK8
klk8_expression <- GetAssayData(merged_human, 
                                slot = "data", 
                                assay = "RNA")["KLK8", ] 
unique(klk8_expression)
table(klk8_expression)
