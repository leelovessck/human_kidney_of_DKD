...
####单独对象的LIF#####################
#单独对象的LIF
seurat_objects <- c(paste0("CON", 1:6), paste0("DKD", 1:6))  
for (obj_name in seurat_objects) {  
  lif_expression <- FetchData(object = get(obj_name), vars = "LIF")  
  cat(sprintf("Object %s - LIF Expression:\n", obj_name))  
  print(lif_expression)  
  cat("\n")  
}
...
####合并对象的LIF#####################
#合并对象的LIF
lif_expression <- GetAssayData(merged_human, 
                                layer = "data", 
                                assay = "RNA")["LIF", ] 
unique(lif_expression)
table(lif_expression)



####EC亚群的LIF#####################
#EC亚群的LIF
EC <- subset(merged_human, subclass.l1 == "EC")
lif_expression <- GetAssayData(EC, 
                               layer = "data", 
                               assay = "RNA")["LIF", ] 
unique(lif_expression)
table(lif_expression)
